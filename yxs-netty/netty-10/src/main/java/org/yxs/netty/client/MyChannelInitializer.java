...
public class MyChannelInitializer extends ChannelInitializer<SocketChannel> {
    ...
    protected void initChannel(SocketChannel socketChannel) throws Exception {
        ...
        //在管道中添加接收数据的方法
        socketChannel.pipeline().addLast(new MyOutMsgHandler());  //消息出站处理器，在Client 发送 消息时候会触发此处理器
        socketChannel.pipeline().addLast(new MyInMsgHandler());   //消息出站处理器，在Client 接收 消息时候会触发此处理器
    }
}
